...
export async function run(provider: NetworkProvider) {
    ...
    const amount = await ui.input('Enter the deposit amount in USDT');
    ...
    await provider.sender().send({
        ...
        body: beginCell().storeUint(op.TransferNotification, 32).storeUint(query.newDepositYToken, 64).endCell(),
    });
